*&---------------------------------------------------------------------*
*& Include:   Z_AI_CUST_AGNT_RPT_F01
...
CLASS lcl_report IMPLEMENTATION.
  ...
  METHOD check_authority.
    " Company code level authorization check (F_BKPF_BUK)
    rv_ok = abap_false.
  ...
  METHOD get_data.
    " Fetch open items from BSID — no SELECT *, only required fields
    ...
    " Fetch customer master data — FOR ALL ENTRIES instead of nested SELECT
  ...
  METHOD calculate_aging.
    " Distribute open items into aging buckets per customer
    DATA ls_output TYPE ty_s_output.

    LOOP AT it_bsid ASSIGNING FIELD-SYMBOL(<ls_bsid>).
      " Skip if customer not found in master data
      ...
      " Accumulate total overdue amount
      IF lv_overdue_days > 0.
  ...
  METHOD calculate_net_due_date.
    " Net due date = baseline date + longest payment term
    ...
    " Use the longest applicable payment term
  ...
  METHOD set_cell_colors.
    " Apply cell colors: red for 90+, yellow for 61-90
  ...
  METHOD add_fcat_entry.
    " Helper to build a single fieldcatalog entry
