FUNCTION zai_001_fm_get_ven_statement.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"----------------------------------------------------------------------
  ...
  DATA ltr_lifnr       TYPE RANGE OF lifnr.
  ...
  IF iv_lifnr IS NOT INITIAL.
    ...
    IF sy-subrc NE 0.
      MESSAGE e015(zai) INTO lv_msg WITH iv_lifnr.
      PERFORM add_syst_mess_to_bapiret2 TABLES lt_return
                                         USING syst
                                               lv_msg.
    ENDIF.
  ...
  IF lt_return IS INITIAL.
    ...
    SUBMIT rfitemap
      WITH kd_lifnr   IN ltr_lifnr
      WITH kd_bukrs   IN ltr_bukrs
    ...
    TRY.
        ...
        IF <fs_datatab> IS ASSIGNED.
          ...
          LOOP AT <fs_datatab> ASSIGNING <fs_datalin>.
